  ...
  export default {
    ...
    data () {
      return {
        ...
        rules: {
          ...
          alphanumeric: v => /^[a-zA-Z0-9-_]+$/g.test(v) || 'Must contain a-z characters only'
        },
      }
    },
    created () {
    ...
    methods: {
      fetchData () {        
        ...
        api.get('sites/' + this.siteId + '/variables')
      ...
      deleteItem(line) {
        this.$confirm('Delete variaable').then(res => {
          if (res) {
            ...
            api.post('sites/' + this.siteId + '/variables', {line: line})
            .then(function (response) {
              console.log(response)
      ...
      saveItem() {
        ...
        if (this.$refs.form.validate()) {
          ...
          api.post('sites/' + this.siteId + '/variables', this.item)
          .then(function (response) {
            console.log(response)
            if (response.data.error) {
              self.fetching = false
