  ...
  export default {
    ...
    methods: {
      ...
      saveKey (key) {
        ...
        // save key
        api.post('servers/' + self.serverId + '/systemusers/' + self.ssh_username, {key: key})
        .then(function (response) {
          console.log(response)
      ...
      saveSSHUser () {
        var self = this
        self.fetching = true
        api.post('servers/' + this.serverId + '/savesshuser', {ssh_username: self.ssh_username})
        .then(function (response) {
          console.log(response)
      ...
      saveKeyAndContinue () {
        ...
        // get ssh key
        api.post('https://shiftedit.net/api/prefs')
        .then(function (response) {
          console.log(response)
      ...
      terminal () {
        if (this.data.ssh_username) {
          ...
          window.open('http://shiftedit.net/ssh/?host=' + this.data.hostname + '&user=' + this.data.ssh_username);
        } else {
          this.fetchUsers()
        }
      }
